        ...
        var $ = options.framework,
            ...
            defaults = {
                ...
                keyValues: {
                    ...
                    "keyTVChDown": VK_KEYPAD_CHANNEL_DOWN, "keySpace": VK_SPACE, "keySource": VK_SOURCE, "keyPadInput": VK_KEYPAD_INPUT, "keyPadMenu1": VK_KEYPAD_ONEKEY_OK,"keyPadMenu5": VK_KEYPAD_FIVEKEY_OK,"keyPadMenu7": VK_KEYPAD_SEVENKEY_OK, "keyFacM": VK_FAC_M, "keyCC": VK_CC, "keySleep": VK_SLEEP,
                    ...
                    "keyFREEZE": VK_FREEZE,
                    "keyCinema":VK_CINEMA,
                    "keyFootbal":VK_FOOTBAL,
                    "keyNECAuto":VK_NEC_AUTO
...
//                debugPrint('_________________keyup');
        ...
        function keyDownHandler(event) {
            if(!tv){
                UIObserver.publishMessage(UIObserver.MESSAGE_NAME.KEY_REPORT, event.keyCode);
            }
...
//                    break;
                ...
                case hiSDKSettings.keyValues.VK_KB_BlankString:
                    ...
                    if (this.PCB == 'button' || this.PCB == 'component') {
                        if ((this.CaEType == 'input' && !!this.isWrite) || (this.page.pageMode.toLowerCase() == 'iqqi' && this.CaEType != 'Container')) {
                            var input = this.page.pageMode.toLowerCase() == 'iqqi' ? this.page.getCaE('iqqiInput') : this;
                            try {
                                ...
                                var keyCharCode = String.fromCharCode(event.keyCode);
                                if (event.keyCode >= VK_a && event.keyCode <= VK_z) {
                                    keyCharCode = String.fromCharCode(event.keyCode - 389);
                                }
                                if (event.keyCode >= VK_KB_SQM && event.keyCode <= VK_KB_RIGHTDASH) {
                                    keyCharCode = String.fromCharCode(event.alphakey);
                                }
                                if (event.keyCode == VK_KB_BlankString) {
                                    keyCharCode = '';
                                }
                                if ((event.keyCode >= VK_a && event.keyCode <= VK_z) || (event.keyCode >= VK_A && event.keyCode <= VK_Z)) {
                                    iqqiWord(this, undefined, this.page, opeData, keyCharCode);
                                } else {
                                    iqqiPunc.call(this, undefined, this.page, opeData, keyCharCode);
                                }
                            }
                            catch (ex) {
                                debugPrint('exit iqqi:' + ex.message, DebugLevel.ERROR);
                            }
                            /*********************  增加输入框的回调函数  ****************************/
...
//                        }
                ...
                case hiSDKSettings.keyValues.keyChannelUp:
                    ...
                    if (this.PCB == 'page') {
                        ...
                        if (!!this.handler && typeof this.handler.keyChannelUpHandler == 'string') {
                            hasDowith = true;
                            eval(this.handler.keyChannelUpHandler + '.call(this,this.page.operateData,this.data)');
                        } else if (!!this.handler && typeof this.handler.keyChannelUpHandler == 'function') {
                            hasDowith = true;
                ...
                case hiSDKSettings.keyValues.keyChannelDown:
                    ...
                    if (this.PCB == 'page') {
                        ...
                        if (!!this.handler && typeof this.handler.keyChannelDownHandler == 'string') {
                            hasDowith = true;
                            eval(this.handler.keyChannelDownHandler + '.call(this,this.page.operateData,this.data)');
                        } else if (!!this.handler && typeof this.handler.keyChannelDownHandler == 'function') {
                            hasDowith = true;
...
//                console.log(o.id + ",数据选中" + o.DataSelectedIndex + ',导航选中' + o.SelectedIndex);
        ...
        function HiButton(data, opts, page, parent) {
            ...
            if(opts.CaEType == "input") {
                ...
                this.inputTitle = opts.inputTitle;
                if(!!opts.inputMethod) {
...
//                                }
                            ...
                            case 'input':
                                ...
                                if (!!tempData.inputTitle)this.inputTitle = !!tempData.inputTitle;
                                var type = 'text';
...
//            console.log("id="+id+",idx="+idx)
        ...
        function registerKeyCodesNormal() {
            keyboard.registerKeyCodes([ VK_LEFT, VK_RIGHT, VK_UP, VK_DOWN, VK_ENTER, VK_MENU, VK_STOP, VK_EPG, VK_F1,VK_POWER_KEY_PAD,
                ...
                VK_KEYPAD_ONEKEY_OK,VK_KEYPAD_FIVEKEY_OK,VK_KEYPAD_SEVENKEY_OK, VK_BT_CONNECT, VK_VOICE,VK_LOWBATTERY,VK_AUDIOONLY,
                VK_A,
                VK_B,
                VK_C,
                VK_D,
                ...
                VK_PIC_CINEMA_NIGHT,VK_PIC_CINEMA_DAY,VK_FAV_CH,VK_PICTURE,VK_S_MODE,VK_PRE_CH,VK_CH_LIST,VK_SND_MOVIE,
                VK_FREEZE,VK_CINEMA,VK_FOOTBAL,VK_NEC_AUTO
            ]);
        ...
        function registerKeyCodesLobster() {
            keyboard.registerKeyCodes([ VK_EPG, VK_MENU, VK_ZOOM, VK_PICTURE,
                VK_PIP, VK_HOME, VK_PVR, VK_EXIT, VK_MEDIA, VK_LIVETV, VK_VOLUME_DOWN, VK_VOLUME_UP, VK_KEYPAD_VOLUME_UP, VK_KEYPAD_VOLUME_DOWN, VK_KPAD, VK_BT_CONNECT,VK_LOWBATTERY,VK_KEYPAD_ONEKEY_OK,VK_KEYPAD_FIVEKEY_OK,VK_KEYPAD_SEVENKEY_OK]);
        }

        function registerKeyCodesExcludeKey(key) {
            var baseKey = [
                VK_HOME, VK_EXIT, VK_LIVETV, VK_MENU, VK_ALLAPP,VK_MEDIA,
                VK_NETFLIX, VK_VUDU, VK_SOURCE, VK_MUTE, VK_YOUTUBE,VK_WUAKI,VK_OPERA,
                VK_VOLUME_DOWN, VK_VOLUME_UP, VK_AMAZON, VK_FAC_M,VK_AUDIOONLY,
                VK_KEYPAD_VOLUME_DOWN, VK_KEYPAD_VOLUME_UP, VK_ASPECT,
                VK_KEYPAD_CHANNEL_DOWN, VK_KEYPAD_CHANNEL_UP, VK_EPG, VK_GUIDE, VK_KPAD, VK_BT_CONNECT,VK_LOWBATTERY,VK_KEYPAD_ONEKEY_OK,VK_KEYPAD_FIVEKEY_OK,VK_KEYPAD_SEVENKEY_OK, VK_POWER_KEY_PAD,
                VK_AV1,VK_AV3,VK_AV2,VK_COMPONENT1,VK_HDMI1,VK_HDMI2,VK_HDMI3,VK_HDMI4,VK_HDMI5,VK_VGA,VK_PICMODE_NORMAL,
                VK_PICMODE_CINEMA,VK_PICMODE_PANORAMA, VK_PICMODE_ZOOM,VK_PICMODE_WIDE, VK_ASPECT_AUTO,VK_ASPECT_DIRECT,
                VK_PIC_CINEMA_NIGHT,VK_PIC_CINEMA_DAY,VK_FAV_CH,VK_PICTURE,VK_S_MODE,VK_PRE_CH,VK_CH_LIST,VK_SND_MOVIE,
                VK_FREEZE,VK_CINEMA,VK_FOOTBAL,VK_NEC_AUTO
        ...
        function registerKeyCodesForAppWithKey(key) {
            var baseKey = [
                VK_HOME, VK_EXIT, VK_LIVETV, VK_MENU, VK_ALLAPP,VK_MEDIA,
                VK_NETFLIX, VK_VUDU, VK_SOURCE, VK_MUTE, VK_YOUTUBE,VK_WUAKI,VK_OPERA,VK_AUDIOONLY,
                VK_VOLUME_DOWN, VK_VOLUME_UP, VK_AMAZON, VK_FAC_M,
                VK_KEYPAD_VOLUME_DOWN, VK_KEYPAD_VOLUME_UP, VK_SLEEP, VK_ASPECT,
                VK_KEYPAD_CHANNEL_DOWN, VK_KEYPAD_CHANNEL_UP, VK_EPG, VK_GUIDE, VK_KPAD, VK_BT_CONNECT,VK_LOWBATTERY,VK_KEYPAD_ONEKEY_OK,VK_KEYPAD_FIVEKEY_OK,VK_KEYPAD_SEVENKEY_OK, VK_POWER_KEY_PAD,VK_AUDIOONLY,
                VK_AV1,VK_AV3,VK_AV2,VK_COMPONENT1,VK_HDMI1,VK_HDMI2,VK_HDMI3,VK_HDMI4,VK_HDMI5,VK_VGA,VK_PICMODE_NORMAL,
                VK_PICMODE_CINEMA,VK_PICMODE_PANORAMA, VK_PICMODE_ZOOM,VK_PICMODE_WIDE, VK_ASPECT_AUTO,VK_ASPECT_DIRECT,
                VK_PIC_CINEMA_NIGHT,VK_PIC_CINEMA_DAY,VK_FAV_CH,VK_PICTURE,VK_S_MODE,VK_PRE_CH,VK_CH_LIST,VK_SND_MOVIE,
                VK_FREEZE,VK_CINEMA,VK_FOOTBAL,VK_NEC_AUTO
        ...
        function registerKeyCodesForSettingOnApp() {
            var baseKey = [
                ...
                VK_NETFLIX, VK_VUDU, VK_YOUTUBE, VK_AMAZON,VK_WUAKI,VK_OPERA,VK_AUDIOONLY,
                VK_SOURCE,VK_CC,
                VK_KEYPAD_VOLUME_DOWN, VK_KEYPAD_VOLUME_UP, VK_FAC_M,VK_KEYPAD_ONEKEY_OK,VK_KEYPAD_FIVEKEY_OK,VK_KEYPAD_SEVENKEY_OK,
