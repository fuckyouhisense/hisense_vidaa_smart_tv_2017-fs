...
function liveTVChannelList() {
    ...
    function playChannel(playId, uid, listUid, num) {
        ...
        livetvmain.setCurrentSourceInnerId(SourceList.TV) ;//切台时，MTK底层会自动切通道到TV，但是不会通知。所以这里重置一下
        if (tv) {
            DBG_ERROR("model.tvservice.playChannel" + (new Date().getTime()));
    ...
    self.changeChannel = function(chFlag, chParam) {
        try {
            ...
            if(tv && tp_common.checkLeaveTimeshiftOrPvr(function () {
                    self.changeChannel(chFlag, chParam);
                })) {
    ...
    self.changeChannelByNumKey = function(num) {
        ...
        var iterval = (!!numChangeList && "COL" != crntArea
    ...
    function rewritePageData(pageData) {
        try {
            ...
            for(var i = 0; i < pageSize; i++) {
                ...
                var tempChnl = !!oprtData.channels[currentList.name] ? oprtData.channels[currentList.name][pos] : null;
                if(tempChnl) {
                    tempNode = {
    ...
    function onGetChannels(list, m_event) {
        if(m_event.type == TableIterator.EVENT_TYPE_ROWS_READ) {
 //           DBG_ERROR("m_event.rows::"+objToString(m_event.rows));
    ...
    function eventRowsToProgram(rows) {
        ...
        for(var i = 0; i < rows.length; i++) {
            ...
            /*var bkFlag = tp_common.getProgramBookType({
                uid: prgrm.channelUid,
                playId: prgrm.playId
            }, prgrm.startTime, prgrm.endTime);*/
            var bkFlag = getIsPvrOrReminder({
                uid: prgrm.channelUid,
                playId: prgrm.playId
            }, prgrm.startTime, prgrm.endTime);
    ...
    function getIsPvrOrReminder( channel, startTime, endTime ) {
        try {
            ...
            bookFlag = { reminder: false, record: false };
    ...
    function resetStyleWhenFlipChInfo(flag) {
        ...
        if(0 == flag) { // focus channel
           ...
           // $(".liveTVChListTiltle").css("background", "url(img/channellist/title_list.png) no-repeat");
            $(".liveTVChListTiltle").css({"background": "url(img/channellist/title_list.png) no-repeat",
                "border-bottom":"solid","border-color":"#808080","border-bottom-width":"1px !important",});
    ...
    self.openEPGPage = function() {
        ...
        try {
            ...
            if(hiWebOsFrame.getCurrentPageId() == LiveTVModule.MAIN &&
                ...
                if(tp_common.checkLeaveTimeshiftOrPvr(openEPGPage)) {
                    return;
                }
                var curList = objectFindByKey(oprtData.channelList, ["uid", "satId"], [curChannel.listUid, curChannel.satId]);
                if(null == curList) {
    ...
    self.checkRolloverDown = function(){
        ...
        if(this.SelectedIndex == oprtData[oprtKey].length - 1){
            if("channelList" == oprtKey){
                for (var i = 0; i < oprtData[oprtKey].length; i++) {
                    if (!!oprtData[oprtKey][i].display && oprtData[oprtKey][i].display == true) {
                        this.setSelectedIndex(i);
                        break;
                    }
                }
                self.restoreListMarquee(this.id, this.id, oprtData[oprtKey].length - 1, i, 300);
            }
            else {
    ...
    self.checkRolloverUp = function(){
        ...
        if(this.SelectedIndex == 0 && oprtData[oprtKey].length > 0){
        ...
        else if (oprtData[oprtKey].length > 0) {
            for (var i = this.SelectedIndex; i > 0; i--) {
                if (oprtData[oprtKey][i-1].display == true) {
                    if("channelList" == oprtKey){
                        self.restoreListMarquee(this.id, this.id, this.SelectedIndex, this.SelectedIndex - 1, 300);
                    }
                    return true;
            ...
            if("channelList" == oprtKey){
                self.restoreListMarquee(this.id, this.id, this.SelectedIndex, oprtData[oprtKey].length - 1, 300);
                this.setSelectedIndex(oprtData[oprtKey].length - 1);
                return false;
            }
