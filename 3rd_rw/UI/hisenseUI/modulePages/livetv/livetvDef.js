...
function liveTVHandlerProcess(keyCode, keytype) {
    DBG_ERROR("live tv receive key GLOBAL.PARENTAL_CHILD_LOCK = " + GLOBAL.PARENTAL_CHILD_LOCK + ", GLOBAL.PARENTAL_LOCK_SWITCH = " + GLOBAL.PARENTAL_LOCK_SWITCH);
    if(GLOBAL.PARENTAL_CHILD_LOCK && GLOBAL.PARENTAL_LOCK_SWITCH){
        DBG_ERROR("tv is lock, ignore this handler!");
        return;
    }
    ...
    if(keyCode == VK_TELETEXT) {
        ...
        srcInnerId >= SourceList.HDMI1 && srcInnerId <= SourceList.HDMI3 && !livetvmain.getGloableLockState()) {// cec
        DBG_ERROR(" sendKeyToCEC(" + keyCode + "); ");
        ...
        switch(keyCode) {
            ...
            case VK_FAST_FWD:
            {
                ...
                if(tp_common.isPvring() || tp_common.isTimeShifting() || (!livetvmain.getNoSignalFlag() && !!crntChannel && crntChannel.type != TVTYPE.ATV && crntChannel.eCode != ECode.AUDIO)) {
                    try {
                        // 补丁 add by ghl
            ...
            case VK_CHANNEL_DOWN:
            {
                ...
                DBG_ERROR(" UI receive down key" + (new Date().getTime()));
                /*if(hiWebOsFrame.getKeyRepeatMode()) {
                    livetvchlist.changeInfoBar(-1);
                }
                else {*/
...
CEC_KEYMAP[VK_VOLUME_UP] = CEC_ENUM++;
CEC_KEYMAP[VK_VOLUME_DOWN] = CEC_ENUM++;
CEC_KEYMAP[VK_MUTE] = CEC_ENUM++;
CEC_KEYMAP[VK_PLAY] = CEC_ENUM++;
CEC_KEYMAP[VK_STOP] = CEC_ENUM++;
CEC_KEYMAP[VK_PAUSE] = CEC_ENUM++;
CEC_KEYMAP[VK_FAST_FWD] = CEC_ENUM++;
CEC_KEYMAP[VK_FAST_BKW] = CEC_ENUM++;
CEC_KEYMAP[VK_LEFT] = CEC_ENUM++;
CEC_KEYMAP[VK_UP] = CEC_ENUM++;
CEC_KEYMAP[VK_RIGHT] = CEC_ENUM++;
CEC_KEYMAP[VK_DOWN] = CEC_ENUM++;
CEC_KEYMAP[VK_ENTER] = CEC_ENUM++;
CEC_KEYMAP[VK_BACK_SPACE] = CEC_ENUM++;