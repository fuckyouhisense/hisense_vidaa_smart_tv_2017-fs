...
var applications = [
    ...
    {"name": "tv_store", "pageId": "app_tv_store","key": VK_OPERA, "type": "native"},
...
var VIDAAU2LauncherCreateFromAllAppsKey = false;
var VIDAAU2LauncherCreateFromSourceKey = false;
var FREEVIEWTEST = false, ENABLE_FVP = false;
var isStartFromNetfliex, isWizardFlag, powerOffTipFlag = -1;
var powerOffTipTimer = 0, powerOffSourceTimer = 0;
var lastClosedPage = "";
var miracastClosedByAppHotkey = false;
...
//    protectAutoClosePages: ['setting_update_progressing_page', "setting_autoupdate_page", "setting_forceupdate_page"],//保护不被关闭的页面
            ...
            protectPages: ['setting_Upgrade_Progress_Page', "settingUpgradeDownloadingOkId", "setting_update_ver_page", "setting_update_fir_page",

                "settingChSetAutoScanPageId", "settingChSetEUAutoScanPageId", "settingChSetDVBTUKAreaListDialogId", "settingChSetDVBTLCNListDialogId", "settingChSetFransatOpListDialogId", "beginRecordDialog", "pvrRecordTip", "livetv_operate_tip",
            ...
            systemKeys: [VK_HOME, VK_EXIT, VK_MEDIA, VK_LIVETV, VK_MENU, VK_FAC_M, VK_AMAZON, VK_VUDU, VK_ALLAPP, VK_YOUTUBE, VK_NETFLIX, VK_SLEEP, VK_SOURCE, VK_KEYPAD_INPUT, VK_F1, VK_KEYPAD_ONEKEY_OK,VK_KEYPAD_FIVEKEY_OK,VK_KEYPAD_SEVENKEY_OK, VK_BT_CONNECT, VK_EPG, VK_KPAD, VK_STICKER, VK_VOICE, VK_AUDIO_DESCRIPTION, VK_WUAKI,VK_OPERA,VK_AUDIOONLY,
                ...
                VK_PICMODE_PANORAMA, VK_PICMODE_ZOOM, VK_PIC_CINEMA_NIGHT, VK_PIC_CINEMA_DAY, VK_ASPECT_AUTO, VK_ASPECT_DIRECT, VK_SND_MOVIE, VK_FREEZE, VK_CINEMA, VK_FOOTBAL, VK_NEC_AUTO],
            keySwitch: true,
            KeyStackLimit: 5,
            ...
            "areas": [//区域划分定义
                {
                    "SA": [
                        {
						"SA": ["Brazil", "Argentina", "Philippines", "Uruguay", "Peru", "Ecuador", "Chile", "Costa Rica","Bolivia","Paraguay","Colombia","Venezuela"]
                        }
                    ]
                },
                //{
...
//            debugPrint("[xuehongfeng] is not ch1_blankpage",DebugLevel.ALWAYS);
            ...
            "defaultClosePageCallback": function (currentModule, beCloseOrigin) {
                ...
                if (currentModule == "setting") {
                    try {
                        ...
                        if(InitArea=="SA"&&GLOBAL.PARENTAL_CHILD_LOCK&&GLOBAL.PARENTAL_LOCK_SWITCH){
            ...
            keyMapFun: function (event) {
                try {
                    debugE("event.keyCode is:" + event.keyCode);
                    clearTimeout(powerOffSourceTimer);
                    if (event.keyCode == hiWebOsFrame.getKeyValues().keyPadMenu1) {
                        if (!HOTElKEYBboardLock) {
                            if (!hiWebOsFrame.getKeyRepeatMode() && hiWebOsFrame.getCurrentPage().module == "mixBar") {
                               // clearTimeout(powerOffTipTimer);
                                try {
                                var page = hiWebOsFrame.getCurrentPage();
                                if((!MixBarKeyMenuOne ||MixBarKeyMenuOne==1)&& page.getCaE('mixBar_Item').SelectedIndex >= MixBarData.operateData.source.length - 1){
                                    MixBarKeyMenuOne++;
                                    event.keyCode = hiWebOsFrame.getKeyValues().keyDown;
                                }else if((MixBarKeyMenuOne == 2 || MixBarKeyMenuOne == 3)&& (page.getCaE('mixBar_Item').SelectedIndex >= MixBarData.operateData.source.length - 1 || page.getCaE('mixBar_Item').SelectedIndex==0)){
                                    page.getCaE('mixBar_Item').SelectedIndex = 0;
                                    MixBarKeyMenuOne++;
                    ...
                    if (event.keyCode == hiWebOsFrame.getKeyValues().keyPadMenu5) {
                        if (!HOTElKEYBboardLock) {
                            if (hiWebOsFrame.getCurrentPage().module == "mixBar") {
                    ...
                    if (event.keyCode == hiWebOsFrame.getKeyValues().keyPadMenu7) {
                        if (!HOTElKEYBboardLock) {
                            if ((hiWebOsFrame.getCurrentPage().module == "setting" || hiWebOsFrame.getCurrentPage().module == "settingfirst") || hiWebOsFrame.getCurrentPageId() == "VIDAAU2TvInput" || hiWebOsFrame.getCurrentPageId() == "VIDAALiteTvInput" || hiWebOsFrame.getCurrentPageId() == "setting_pic_Source_page") {
                                event.keyCode = hiWebOsFrame.getKeyValues().keyEnter;
                            }
                            else {
                        ...
                        if (event.keyCode == hiWebOsFrame.getKeyValues().keyPadInput) {
                            if (!HOTElKEYBboardLock) {
                                ...
                                else if ((hiWebOsFrame.getCurrentPage().module == "setting" || hiWebOsFrame.getCurrentPage().module == "settingfirst" || hiWebOsFrame.getCurrentPage().module == "mixBar_page")) {
                                    event.keyCode = hiWebOsFrame.getKeyValues().keyEsc;
                                }
                                else {
                        ...
                        if (event.keyCode == hiWebOsFrame.getKeyValues().keyTVChDown) {
                            if (!HOTElKEYBboardLock) {
                                if ((hiWebOsFrame.getCurrentPage().module == "setting" || hiWebOsFrame.getCurrentPage().module == "settingfirst") || hiWebOsFrame.getCurrentPageId() == "VIDAAU2TvInput" || hiWebOsFrame.getCurrentPageId() == "VIDAALiteTvInput" || hiWebOsFrame.getCurrentPageId() == "mixBar_page") {
                                    event.keyCode = hiWebOsFrame.getKeyValues().keyDown;
                                }
                            } else {
                                event.keyCode = 0;
            ...
            winKeyDownHandler: function (event) {
                ...
                if (TeletextDevice.status == 2) {
                    ...
                    if (event.keyCode == hiWebOsFrame.getKeyValues().keyExit) {
                        ...
                        || event.keyCode == hiWebOsFrame.getKeyValues().keyOpera
                ...
                switch (event.keyCode) {
                    ...
                    case hiWebOsFrame.getKeyValues().keyOpera:
...
//                }
                        ...
                        if (event.keyCode == VK_NETFLIX) {
                            if(GLOBAL.HOTEL_MODE){
                                DBG_ERROR("GLOBAL.HOTEL_MODE key netflix not handle");
                                return false;
                            }
                        ...
                        if (hiWebOsFrame.getCurrentPageId() == "setting_pic_Mira_page" || hiWebOsFrame.getCurrentPageId() == "setting_Mira_exit_page"
                            || hiWebOsFrame.getCurrentPageId() == "setting_Mira_wifi_page") {
                            miracastClosedByAppHotkey = true;
                            DBG_ERROR("now page is " + hiWebOsFrame.getCurrentPageId() + "  need change miracastClosedByAppHotkey!!");
                        }

                        //进第三方应用，若当前有录制时，UI提示
                        if (tp_common.checkLeaveTimeshiftOrPvr(appKeyStopPvrOrTShiftCallBack.bind(this, 0, event.keyCode))) {
                        ...
                        if (hiWebOsFrame.getCurrentPageId() == app_c.pageId) {
                            ...
                            if(event.keyCode == VK_OPERA){
                                asyncStartApp(app_c.pageId, app_c.name, true, false, false, true,StoreType.OPERA);
                    ...
                    case hiWebOsFrame.getKeyValues().keySource:
                        ...
                        if (g_launcherBrand == "VIDAALiteU2") {
                            ...
                            //进home，若当前有录制时，UI提示
                            if (tp_common.checkLeaveTimeshiftOrPvr(homeSourceKeyStopPvrOrTShiftCallBack)) {
                                debugPrint("keyHome is pressed!");
                                return;
                            }
                            //tempDebug = false;//正式去掉
                            if (!canCurrentPageProcEvent())return false;
                            if (hiWebOsFrame.isCurrentModule("launcher") && hiWebOsFrame.getCurrentPageId() == "VIDAAU2TvInput") {
                        ...
                        break;
                    case hiWebOsFrame.getKeyValues().keyPadMenu5://add for mixbar 0227
                        ...
                        break;
                    case hiWebOsFrame.getKeyValues().keyPadMenu1://add for mixbar 0227
                    ...
                    case hiWebOsFrame.getKeyValues().keyMenu:
                        try {
                            if (!tv) return;
                            if(g_toastType == ToastType.NETWORKSET && g_toastIsShow){
//                                closePagesOrModuleByPage(hiWebOsFrame.getCurrentPage());
                                hideNotificationToast();
                                LauncherquickopenSetting(3, 0, hiWebOsFrame.getCurrentPage());
                                return;
                            }else if(g_toastType == ToastType.DLNA && g_toastIsShow){
//                                closePagesOrModuleByPage(hiWebOsFrame.getCurrentPage());
                                hideNotificationToast();
                                hiWebOsFrame.createPage("DLNAMessage", null, hiWebOsFrame.getCurrentPage(), null, function (a) {
                                    hiWebOsFrame.DLNAMessage = a;
                                    a.open();
                                    a.hiFocus();
                                });
                                return;
                        ...
                        try {
                            ...
                            if (hiWebOsFrame.isCurrentModule("setting") || hiWebOsFrame.isCurrentModule("settingfirst") || hiWebOsFrame.isCurrentModule("hotel")) {
                                ...
                                if (tp_common.checkLeaveTimeshiftOrPvr(menuKeyStopPvrOrTShiftCallBack)) {
...
//                closeMsgPage(false);
                    ...
                    case hiWebOsFrame.getKeyValues().keyHome:
                        ...
                        //进home，若当前有录制时，UI提示
                        if (tp_common.checkLeaveTimeshiftOrPvr(homeKeyStopPvrOrTShiftCallBack)) {
                            debugPrint("keyHome is pressed!");
                            return;
                        }
                        //tempDebug = false;//正式去掉
                        ...
                        if (hiWebOsFrame.isCurrentModule("launcher")) {
                            if (g_launcherBrand == "SA_OEM" || g_launcherBrand == "VIDAALiteU2") {
                                var crntPage = hiWebOsFrame.getCurrentPage();
                                if(hiWebOsFrame.getCurrentPageId() == "VIDAAU2TvInput" || hiWebOsFrame.getCurrentPageId() == "VIDAAU2AppPage" || hiWebOsFrame.getCurrentPageId() == "VIDAAU2NobAppPage"
                                    || hiWebOsFrame.getCurrentPageId() == "notification"){
                                    crntPage.close();
                                    if(hiWebOsFrame.isPageExist("VIDAAU2MainTitlePage") || hiWebOsFrame.isPageExist("NoblexLauncher_page")){
...
//                                            },10);
                            ...
                            if (null == crntPage) {
                                ...
                                if (g_launcherBrand == "SA_OEM") {
                                    ...
                                    hiWebOsFrame.createPage(id, null, null, null, function (a) {
                                        hiWebOsFrame.myLauncher = a;
                                        hiWebOsFrame.createPage("VIDAAU2StatusPage", null, null, null, function (page) {
                    ...
                    case hiWebOsFrame.getKeyValues().keyAllApp:
                        ...
                        if (tp_common.checkLeaveTimeshiftOrPvr(allAppKeyStopPvrOrTShiftCallBack)) {
                    ...
                    case hiWebOsFrame.getKeyValues().keyLiveTV:
                        ...
                        if (defaultSource != livetvmain.getCurrentSource().uid && (hotelMode != 1 || ((hotelMode == 1) && (!livetvmain.getSourceByUid(defaultSource).hotelLock)))) {
                            if (tp_common.checkLeaveTimeshiftOrPvr(liveTvKeyStopPvrOrTShiftCallBack)) {
                    ...
                    case hiWebOsFrame.getKeyValues().keyPMode:
                        if(hiWebOsFrame.getCurrentPage().module == "wizard")return;
                        try {
                            ...
                            if ("SA" == InitArea) {
                                ...
                                if (!!hiWebOsFrame.ginga_app && curPage.id == hiWebOsFrame.ginga_app.id) {
                                    ...
                                    if ('brazil' == hiWebOsFrame.getCurrentCountry().toLowerCase() || 'argentina' == hiWebOsFrame.getCurrentCountry().toLowerCase()) {
                                        ...
                                        showMsg('', 'The Ginga service is not currently available in this country.');
                                    }
                    ...
                    case hiWebOsFrame.getKeyValues().keyPowerKeyPad:
                        ...
                        break;
                    case hiWebOsFrame.getKeyValues().keyCinema:
                        keyCinemaHandler();
                        break;
                    case hiWebOsFrame.getKeyValues().keyFootbal:
                        keyFootbalHandler();
                        break;
                    case hiWebOsFrame.getKeyValues().keyNECAuto:
                        if(hiWebOsFrame.getCurrentPage().module == "wizard")return;
                        closeDOthersModule("");
            ...
            winKeyUpHandler: function (event) {
                ...
                if (!!hiWebOsFrame.getCurrentPage() &&
                    ...
                    if (event.keyCode == VK_CHANNEL_DOWN || event.keyCode == VK_CHANNEL_UP ||
                        ...
                        DBG_ERROR(" UI receive keyUP" + (new Date().getTime()));
...
//           openChannelManager();
            UIObserver = new MessagePool();
            UIObserver.getNotifications();
//            UIObserver = new MessagePool();
            //UIObserver.getNotifications();
            hiWebOsFrame.setCurrentBrand("nob");
            hiWebOsFrame.setLanguage("eng");
            createVIDAALiteU2launcher();
           // onkeySourceOpenNobLauncher();
//            initsystemid();
            //createVIDAALitelauncher();
            //return;
...
//            startSetting();
            //startePos(true);
//            hiWebOsFrame.createPage("VIDAAU2NobAppPage", null, null, null, function (a) {
//                hiWebOsFrame.VIDAAU2AppPage = a;
//                a.open();
//                a.hiFocus();
//            });
//            hiWebOsFrame.createPage("NoblexLauncher_page", null, null, null, function (a) {
//                hiWebOsFrame.NoblexLauncher_page = a;
//                a.open();
//                a.hiFocus();
//            });
...
//	            hiWebOsFrame.setCurrentBrand(initBrand);    //防止出现config比model先加载的情况出现
                ...
                UIObserver.subscribeMessage(UIObserver.MESSAGE_NAME.CURRENT_COUNTRY_CHANGED, onTvsetLocationChaged);
...
function doTVScale() {
    try {
        ...
        $('#page_1080').css('transform', 'scale(0.6667)');
        $('#page_1080').css('transform-origin', 'top left');
...
function doTVInit() {
    GLOBAL.HOTEL_MODE = model.hotel.getHotelMode();
    DBG_INFO("doTVInit:GLOBAL.HOTEL_MODE=" + GLOBAL.HOTEL_MODE);
    GLOBAL.APP_POWER_ON = parseInt(model.system.getOpenFromStandby());
    DBG_INFO("doTVInit:GLOBAL.APP_POWER_ON = " + GLOBAL.APP_POWER_ON);
    if(GLOBAL.HOTEL_MODE){
        if(0 != model.system.getUserMode()){
            model.system.setUserMode(0);
        }
        model.system.setOpenFromStandby(0);
        GLOBAL.APP_POWER_ON = 0;
        isStartFromNetfliex = false;
    }else{
        isStartFromNetfliex = (1 == GLOBAL.APP_POWER_ON);

    }

    isWizardFlag = getWizardSetFlag();
    DBG_ERROR("isWizardFlag is " + isWizardFlag);
    doTVScale();
    if(!isStartFromNetfliex){
        DBG_ERROR("stop animation by init.");
        model.system.stopAnimation(0);
    }
    model.cec.setArcEnable(1);
    //from 5658
    setTimeout(function () {
...
function checkStoreMode() {
    if (!tv) {
        ...
        if(model.system.getUserMode() != 0) {
            DBG_INFO("currUserMode is Stor");
            return true;
        }
        else{
            return false;
...
//	    SystemInitBrand();
        ...
        if (0 == val && !isFileExist("his_factory_do_ok_over", 0)) {
            ...
            if (((isWizardFlag == 2 ) || (isWizardFlag == 1))) { //if start from setting-reset and factory ok
        ...
        DBG_ERROR("volume_mute visibility" + $("#volume_mute").css("visibility"));
    }
    catch (ex) {
        DBG_ERROR(ex.message);
    }
...
//    model.network.onEnumNetworkAvailableChaged = onNetworkStateChanged;
    UIObserver.subscribeMessage(UIObserver.MESSAGE_NAME.NETWORK_CHANGED, onNetworkStateChanged);
    ...
    model.system.onEnterSuspendChanged = onEnterSuspendChanged;
    //model.fvpepg.onFEPGAppURLChanged = onFEPGAppURLChanged;//@shixin
}
...
function startLiveTv() {
    ...
    try{
        DBG_ERROR("fte logreport ");
        if(model.network.getEnumNetworkAvailable() == 1 &&  model.network.getEnumNetworking() == 1){
            LGReport.GTFTE("Wireless");
        }else if(model.network.getEnumNetworkAvailable() == 1 &&  model.network.getEnumNetworking() == 0){
    ...
    PingNewsanUrlForPoweron();
}

var hasPingNewsanUr = false;
function doSomethingInitForWizard() {
    initDlnaMode();
    HeadphoneInsertInit();
}
function PingNewsanUrlForPoweron() {
    if (!tv) {
        debugPrint('________________________oemLauncherPingNewsanUrl_______________________');
        return;
    }
    var currentBrand = hiWebOsFrame.getCurrentBrand();
    var brandList = ['nob', 'jvc', 'san', 'phi', 'ilo', 'pio', 'sei', 'ton', 'psc', 'shp'];
    if ($.inArray(currentBrand, brandList) == -1) {
        return;
    }
    if (getNetworkSatus()) {
        debugPrint('________________________PingNewsanUrlForPoweron_______________________' + hasPingNewsanUr);
        if (hasPingNewsanUr) {
            return;
        }
        hasPingNewsanUr = true;

//        var mac = model.network.getMac_address();
        var mac = readFileFromNative('am_tv_mac_address', 0);
        var NewsanUrl = "http://newsan.tv/api/v4/devices/";
//        var customer = Hisense.File.read("am_tv_customer", 0);
        var askAddress = NewsanUrl + mac;
        DBG_ERROR('send url to Newsan server:' + askAddress);
        $.ajax({
            type: "GET",
            url: askAddress,
            timeout: 7000,
            success: function (data) {
//                debugPrint("Newsan server return: " + objToString(data));
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                debugPrint("send url to Newsan server: " + "Get  Newsan server data error!");
                return;
            }
        });
    }
    else {
        DBG_ERROR("send url to Newsan server: Network is unavailable!");
        return;
    }
}
function doSomethingInit() {
    ...
    try {
        ...
        usbNfyTimer = setTimeout(function () {
            ...
            debugPrint("USB_DEVICE_CHANGED register");
        }, 20000);
        ...
        try{
            ...
            debugPrint("showNetworkToastNeverSet "+ e.message);
        }
    ...
    //添加获取platform信息的接口
    initPlatformParam();
    //ADD FOR HBBTV BY ZhaoTongqing
//    try {
//        sendAM(":am,opera4x,hbbtv:starthbbtv=1");
//    } catch (ex) {
//        DBG_ERROR(ex.message);
//    }
    modeljs_adapter.init(); //for pvr && timeshift
    startOpera4x();
}
...
function startLiveTvFirst() {
    ...
    try{
        ...
        DBG_ERROR("mute " + ex.message);
    }
    // debugAlways("gui loaded");
    tv && keyboard.set_listen(1);
    doSomethingInit();
    if (GLOBAL.APP_POWER_ON != 0) {
        closeLiveTVModule();
        LGReport.GTAPPRunHotKey(true);
        model.system.setOpenFromStandby(0);
        DBG_ERROR("startLiveTvFirst "+"GLOBAL.APP_POWER_ON "+GLOBAL.APP_POWER_ON);
        switch (GLOBAL.APP_POWER_ON){
            case GLOBAL.STAND_BY_NETFLIX:
                var curAllAppData = SettingGetApplist();
                var countrycode = model.basicSetting.getTvsetLocation();
                var currentArea = hiWebOsFrame.getCurrentArea();
                DBG_INFO("curAllAppData:" + JSON.stringify(curAllAppData) + ",curArea:" + currentArea + ",curCountry:" + countrycode);
                ...
                break;
            case GLOBAL.STAND_BY_YOUTUBE:
                asyncStartApp("app_youtube", "youtube", false, false, true, true);
                break;
            case GLOBAL.STAND_BY_AMAZON:
                asyncStartApp("app_amazon", "amazon", false, false, true, true);
                break;
            default :
                DBG_ERROR("GLOBAL.APP_POWER_ON = " + GLOBAL.APP_POWER_ON);
                break;
    ...
    setTimeout(function(){startOpera4x();},7000);//MT5659SA-2556
...
function getcurLang() {
    ...
    try {
        ...
        GLOBAL.CURRENT_LANGUAGE = osdlanglist[index].code;
    }
    catch (e) {
        DBG_ERROR(e.message)
    }
...
function getmenutimeout() {
    try {
        GLOBAL.SETTING_TIMEOUT_DELAY= model.basicSetting.getMenuDelayDisappear();
        debugPrint(" GLOBAL.SETTING_TIMEOUT_DELAY" + GLOBAL.SETTING_TIMEOUT_DELAY);
        hiWebOsFrame.setAutoCloseTimeLength(GLOBAL.SETTING_TIMEOUT_DELAY);
    }
    catch (e) {
        DBG_ERROR(e.message)
    }
...
function startQuickSetting(origin) {
    if(InitArea=="SA"&&GLOBAL.PARENTAL_CHILD_LOCK&&GLOBAL.PARENTAL_LOCK_SWITCH){
        hiWebOsFrame.unLockAllKeys("setting");
        LauncherquickopenSetting(4, SYS_SECURITY, origin);
    }
    else {
...
function is_remotepair_show_wizard() {
    return false;
}
...
//    }
            ...
            try{
                ...
                DBG_ERROR("closePagesOrModuleByPage:"+ex.message);
            }
        }
    ...
    try {
        ...
        $.each(destroyList, function (k, v) {
            ...
            switch (v) {
                ...
                case "launcher":
                    if (!!hiWebOsFrame.myLauncher) {
                        if (g_launcherBrand == "SA_OEM" || g_launcherBrand == "VIDAALiteU2" || g_launcherBrand == "VIDAALite") {
                            hiWebOsFrame.myLauncher.close();
                        } else {
                            launcherNBar.closeLauncher();
                        }
...
function oneKeyOpenVIDAAU2AllAppsPage() {
    LogReportData.LauncherRunCloseReason=1;
    closePagesOrModuleByPage(hiWebOsFrame.getCurrentPage());
    tryToCloseLauncher();
    VIDAAU2LauncherCreateFromAllAppsKey = true;
    VIDAAU2LauncherCreateFromSourceKey = false;
    var id = "VIDAAU2AppPage";
    if(hiWebOsFrame.getCurrentBrand() == "nob"){
...
//                }
                ...
                var url = "/3rd_rw/UI/hisenseUI/modulePages/EPOS_new/epos.html,-s,epos";
...
//                debugPrint("[epos]gCurrentZoomIndex = "+gCurrentZoomIndex, DebugLevel.ALWAYS);
                    ...
                    asyncStartApp("app_4k-epos", "file:///3rd_rw/UI/hisenseUI/modulePages/EPOS_new/epos.html", false,null,null,false);
...
//                closePagesOrModuleByPage(hiWebOsFrame.getCurrentPage());
            Hisense.File.write(RETAILMODE_FLAG_FILE, "close", 0); //write data to /tmp/retailmode. open: running, close: stopped
            asyncStopApp("app_4k-epos", hiWebOsFrame.blankPage);
        }
    }
}
function startRetailmodeTimer(enable) {
    ...
    if (enable) {
        var outTime = 60 * 1000;
        if(!!hiWebOsFrame["setting_epos_countdown_page"]) {
            outTime = 120 * 1000;
        }
        ...
        m_retailmodeTimer = setTimeout(function () {
            ...
            startRetailmodeTimer(true);
        }, outTime);//enter into storemode if no remote action within 1 minutes.
...
var onStateOpenChaged = function (val) {
    ...
    if (0 != val) {
        ...
        if (factoryMode == FACTORY_MODE_ENUM.MODE_NORMAL) {
            try {
                model.tvservice.getChannelListUpdate(0);    //为了规避问题底层更改，原来set，现在改为用get实现set。@wangguilan @jianglongsheng
            }
            catch (ex) {
                DBG_ERROR(ex.message);
            }
...
function onUsbStateChanged(val) {
    try {
                ...
                switch (val[0]) {
                    case "online":
                        ...
                        if (!eposPageData.operateData.isDMPPlaying) {
                            if (InitArea == "EU" || InitArea == "EM" || InitArea == "SA") {
        ...
        else if ("offline" == val[0] && "setting_update_dialog2_page" == hiWebOsFrame.getCurrentPageId()) {
           if( !!hiWebOsFrame.settingsupdatedialog2){
               hiWebOsFrame.settingsupdatedialog2.close();
               hiWebOsFrame.settingssysupdate.open();
               hiWebOsFrame.settingssysupdate.hiFocus();
               hiWebOsFrame.settingsupdatedialog2.destroy();
           }
        }
...
function popuUpUsbDialog() {
    ...
    try {
        standby_mode = !!model.system.getBgmStatus();
    } catch (ex) {
        DBG_ERROR("model.system.getBgmStatus err=" + ex.stack);
    }
    if(standby_mode) {
        DBG_ERROR("[popuUpUsbDialog] now is standby mode, do not popup usb dialog");
        return;
    }
...
function onNetworkStateChanged(event) {
    ...
    PingNewsanUrlForPoweron();
    try{
        if(event == NetWorkConnect.CONNECT){
...
function CreateSchdulePvrdialog(origin, prgram, callback) {
...
function canDialogPopUp() {
    ...
    var popup = canCurrentPageProcEvent()
        && !checkIsAppOn()
        && !(GLOBAL.PARENTAL_CHILD_LOCK && GLOBAL.PARENTAL_LOCK_SWITCH)
...
function onMpCtrlStatchanged(value) {
    ...
    switch (parseInt(value)) {
        ...
        case MpCtrlModelDefines.SL2_TVAPI_I32_MPCTRL_ENUM_STATE_AUTO_STOP:
            ...
            if (getCurrVeraForWizard() == 'EU' && hiWebOsFrame.getCurrentPage().module == "wizard") {
                var musicPath = "file:///3rd_rw/UI/hisenseUI/wizard_bg.mp3";
                model.mpctrl.PlayMusic(musicPath);
            }
            else {
...
//                model.system.setReturnlocalappFlag(FlagShareInBrowser.HIMEDIA_PAUSE_TO_SETTING);
        ...
        if ("app_netflix" == crntPage.id) {
            DBG_ERROR("open menu on netflix.");
            try {
                sendAM(":dtv_app_mtk,netflix,:unfocus=netflix");
            } catch (ex) {
                DBG_ERROR(ex.message);
            }
...
function settingOnDestroyCallback(tempOri) {
    ...
    if (!!tempOri) {
        ...
        if ("app" == tempOri.module) {
            ...
            } else if ("app_netflix" == tempOri.id) {
                DBG_ERROR("close menu on netflix.");
                try {
                    sendAM(":dtv_app_mtk,netflix,:focus=netflix");
                } catch (ex) {
                    DBG_ERROR(ex.message);
                }
            }
...
function setSATimezoneOffset() {
    if ("SA" == hiWebOsFrame.getCurrentArea()) {
        hisenseUITZSeconds = parseInt(model.timerfunc.getDeviation());
        DBG_INFO("SA timezone call getDeviation");
        ...
        if (flag) {
            halfAnHourtmr = setInterval(function () {
                if ("open" == readFileFromNative(RETAILMODE_FLAG_FILE, 0)) {
                    DBG_ERROR("action half an hour on.")
    ...
    hiWebOsFrame.createPage(pageId, null, null, idx, function (page) {
        ...
        switch (pageId) {
            ...
            case "VIDAALiteNavPage":
                ...
                break;
            case "VIDAAU2StatusPage":
                DBG_ERROR("preload VIDAAU2StatusPage success!!!!!!!!!!!!!!!!!!");
                var id = "VIDAAU2MainTitlePage";
                if(hiWebOsFrame.getCurrentBrand() == "nob"){
                    id = "NoblexLauncher_page";
                }
                createLauncherPage(id, null, null);
                break;
            case "VIDAAU2MainTitlePage":
            case "NoblexLauncher_page":
                hiWebOsFrame[pageId] = null;
                hiWebOsFrame.myLauncher = page;
                DBG_ERROR("preload VIDAAU2MainTitlePage success!!!!!!!!!!!!!!!!!!");
...
//                    resetStateIfError(null);//预加载原来为func。暂时改为null
...
//                    } else if ('VIDAALiteU2' == g_launcherBrand) {
//                        createLauncherPage('VIDAAU2MainTitlePage', func, null);
////                        if (null != func) func();
//                    }
//                    else {
...
//                    preloadLauncher();
//                }
                DBG_ERROR("g_launcherBrand:"+g_launcherBrand);
                if ('VIDAALite' == g_launcherBrand) {
                    createLauncherPage('VIDAALiteNavPage', null, null);
                    if (null != func) func();
                } else if ('VIDAALiteU2' == g_launcherBrand) {
                    createLauncherPage('VIDAAU2StatusPage', null, null);
                    if (null != func) func();
                }
...
function preloadLauncher() {
    ...
    preloadLauncherTimer = setTimeout(function () {
        ...
        DBG_INFO("preload launcher.:" + g_launcherBrand);
        launcherCreateState = CREATE_STATE.CREATING;
...
var strCallbackExcuted = false;
function onEnterSuspendChanged(val) {
    DBG_ERROR("onEnterSuspendChanged:" + val);

    strCallbackExcuted = true;
    if (0 == val) {
        return;
    }
    else if (2 == val) {
        DBG_ERROR("onEnterSuspendChanged: val is 2, ready to suspend" + hiWebOsFrame.getCurrentPageId());
        GLOBAL.READY_SUSPENDING = true;
        GLOBAL.WAIT_POWERON = GLOBAL.TV_SUSPENDING = true;
        //NRTV逻辑暂时注释掉
//        if (checkIsAppOn()) {
//            GLOBAL.LAST_APPCMD = appControl.getCurrentAPPCMD();
//            DBG_INFO("GLOBAL.LAST_APPCMD: " + GLOBAL.LAST_APPCMD);
//            if (GLOBAL.LAST_APPCMD == APPNAME.NETFLIX) {
//                writeFileToNative("NRTV_NETFLIX", "1", 0);
//                writeFileToNative("NRTV_NETFLIX_1", "1", 0);
//            }
//
//        }
        if (checkIsAppOn()||
            hiWebOsFrame.getCurrentPage().module == "miracast"||
            hiWebOsFrame.getCurrentPageId() == "dlna_picPlayer" ||
            hiWebOsFrame.getCurrentPageId() == "dlna_videoPlayer" ||
            hiWebOsFrame.getCurrentPageId() == "dlna_musicPlayer") {
            DBG_INFO("write SuspendChangedDone!");
            Hisense.File.write("SuspendChangedDone", "1", 0);
        }
        return;
    }

    DBG_ERROR("--STR : UI onEnterSuspendChanged is start!--");
...
//    UI.DOINGSTR = true;
    ...
    closeDOthersModule("");
    if(!!hiWebOsFrame.settingsFirst){
        hiWebOsFrame.settingsFirst.origin = null;
    }
//    UI.DOINGSTR = false;
//    UI.POP_NOSHOW = false;
...
//        if (!!GLOBAL.LAST_APPCMD||checkIsAppOn()) {
...
//        sysUpgradeCancelnotify();
        //  sysOadUpgradeCancelnotify();
//        UI.clearFuncQueue();
//        UI.clearEposMenuTimeout();
//        if(!!timerCheckAppForEpos){
//            clearInterval(timerCheckAppForEpos);
//        }
//        //清除一键开机的timeout
...
//    UI.StopMpctrl = setTimeout(function() {
//        model.mpctrl.StopMpctrl(null);
//    }, 100);
    UI.clearFuncQueue();
    sysUpgradeCancelnotify();
    hiWebOsFrame.unLockAllKeys("STR OFF");
    hiWebOsFrame.registerKeyCodesNormal();
...
function recoverTVState() {
    DBG_ERROR("--STR : UI recoverTVState is start!--");
    //hiWebOsFrame.startLoading(2, "--STR");
    if (!GLOBAL.WAIT_POWERON) {
        //liveTV.recheckCurrentSource();
        return false;
    }
    GLOBAL.WAIT_POWERON = GLOBAL.TV_SUSPENDING = false;
    GLOBAL.READY_SUSPENDING = false;
    GLOBAL.UNLOCKED = false;
    GLOBAL.DROP_NETFLIX_STOP = false;
    if (!strCallbackExcuted) {
        closeDOthersModule("");
        strCallbackExcuted = true;
    }

//    if (UI.StopMpctrl != null) {
//        clearTimeout(UI.StopMpctrl);
//        UI.StopMpctrl = null;
//    }
//    UI.DOINGSTR = true;
//    UI.popAllModule();
...
//        UI.initLanguage(GLOBAL.CURRENT_LANGUAGE);
//    }
...
//    launcher.refreshLauncherData();
    try {
        GLOBAL.HOTEL_MODE = model.hotel.getHotelMode();
        DBG_INFO("recoverTVState:GLOBAL.HOTEL_MODE=" + GLOBAL.HOTEL_MODE);
        GLOBAL.APP_POWER_ON = parseInt(model.system.getOpenFromStandby());
        if(GLOBAL.HOTEL_MODE){
            if(0 != model.system.getUserMode()){
                model.system.setUserMode(0);
            ...
            isStartFromNetfliex = (1 == GLOBAL.APP_POWER_ON);
            DBG_ALWAYS("recoverTVState:GLOBAL.APP_POWER_ON = " + GLOBAL.APP_POWER_ON);
        }

        GLOBAL.CEC_CONTROL = parseInt(model.cec.getFunctionality());
        GLOBAL.CEC_DEVICE_EXIST = parseInt(model.cec.getIsAudioDeviceExist());
        GLOBAL.CEC_ARC_STATE = parseInt(model.cec.getHdmiDevicesArcState());
    } catch (ex) {
        DBG_ERROR("STR ERROR:" + ex.message);
    }

    var cover = tv ? (getWizardSetFlag() == 1) : false;
    DBG_ERROR("recoverTVState: cover is " + cover);
    DBG_ERROR("GLOBAL.APP_POWER_ON = " + GLOBAL.APP_POWER_ON);
//    hasPingNewsanUr = false;
//    try{
//        PingNewsanUrlForPoweron();
//    }
//    catch (ex){
...
//    }
    if (cover) {
        startWizardFromboot();
        GLOBAL.LAST_APPCMD = null;
        keyboard.setWantGroup(7);
        hiWebOsFrame.unLockAllKeys("STR ON");
        GLOBAL.APP_POWER_ON = 0;
    }
    else if (0 != GLOBAL.APP_POWER_ON) {
...
//
//            //setTimeout(function(){
//            //    model.tts.ActionCmd(2,HOTEKEY_MAP.APPNAME[GLOBAL.APP_POWER_ON],50);
//            //}, 2000);
//
//            //解决问题MT5658USATTS-1854,在NETFLIX关机，然后NETFLIX一键开机时不处理STOP
//            if(GLOBAL.LAST_APPCMD==APPNAME.NETFLIX && HOTEKEY_MAP.APPNAME[GLOBAL.APP_POWER_ON]==APPNAME.NETFLIX){
//                GLOBAL.DROP_NETFLIX_STOP = true;
//            }
//            GLOBAL.LAST_APPCMD = null;
...
//                        openAppControl(HOTEKEY_MAP.APPNAME[GLOBAL.APP_POWER_ON], HOTEKEY_MAP.APPNAME[GLOBAL.APP_POWER_ON], CMDType.NATIVEAPP, CMDMethod.STARTUP, false, null);
//                    }
//
//                    break;
//
...
//        openAppControl(HOTEKEY_MAP.APPNAME[GLOBAL.APP_POWER_ON], null, CMDType.NATIVEAPP, CMDMethod.STARTUP, false, null);
        ...
        if (model.system.getUserMode() == 1 || model.system.getUserMode() == 2) {
            DBG_INFO("currUserMode is Store Mode");
            GLOBAL.LAST_APPCMD = null;
            writeFileToNative(RETAILMODE_FLAG_FILE, "close", 0);
            //有了弹出框就不需要直接启动timer了
            //UI.refreshEposMenuTimeout();
            ...
            DBG_ALWAYS("GLOBAL.LAST_APPCMD:  " + GLOBAL.LAST_APPCMD);
            if (!!GLOBAL.LAST_APPCMD) {
//                if(GLOBAL.LAST_APPCMD==APPNAME.NETFLIX){
//                    GLOBAL.LAST_APPCMD = null;
...
//                    UI.initLiveTV(false);
//
//                    openMsgBoxByType(MsgBoxType.APPRESUME_TIP);
//                }
            } else {
                showLiveTVUI();
                resumeDTV();
...
//            DBG_INFO("NEED_START_OPERA4X");
//            sendAM(":am,am,:start=opera4x");
//            GLOBAL.NEED_START_OPERA4X = false;
//        }
//    }, 5000);

    // if (tv)model.source.setDetectSourceResumeenter(100);
    // sysUpgradeInit();
//    setTimeout(function () {
//        recoverUpgradenotify();
//    }, 3000);

//    setTimeout( function(){recoverOadUpgradenotify();
//    },3000);
...
function recoverAppState() {
    ...
    GLOBAL.HOTEL_MODE = model.hotel.getHotelMode();
    GLOBAL.APP_POWER_ON = parseInt(model.system.getOpenFromStandby());
    DBG_INFO("recoverAppState:GLOBAL.HOTEL_MODE=" + GLOBAL.HOTEL_MODE);
    if(GLOBAL.HOTEL_MODE){
        model.system.setUserMode(0);
        model.system.setOpenFromStandby(0);
        GLOBAL.APP_POWER_ON = 0;
        ...
        GLOBAL.APP_POWER_ON = parseInt(model.system.getOpenFromStandby());
        isStartFromNetfliex = (1 == GLOBAL.APP_POWER_ON);
        DBG_ALWAYS("recoverAppState:GLOBAL.APP_POWER_ON = " + GLOBAL.APP_POWER_ON);
    }
    if (isStartFromNetfliex || lastStateNetflix) {
        lastStateNetflix = false;
        model.system.setOpenFromStandby(0);
        asyncStartApp("app_netflix", "netflix", false, false, isStartFromNetfliex, true);
    }
    usbNfyTimer = setTimeout(function () {
        //model.usb.onVstrLatestEventChaged = onUsbStateChanged;
        UIObserver.subscribeMessage(UIObserver.MESSAGE_NAME.USB_DEVICE_CHANGED, onUsbStateChanged);
...
function onKeySourceOpenVIDAAU2InputsPage(origin) {
    hiWebOsFrame.createPage("VIDAAU2TvInput", null, origin, null, function (a) {
        hiWebOsFrame.VIDAAU2TvInput = a;
        a.open();
        a.hiFocus();
    })
}
function onkeySourceOpenNobLauncher(){
    hiWebOsFrame.createPage("NoblexLauncher_page", null, null, null, function (page) {
        hiWebOsFrame.myLauncher = page;
        hiWebOsFrame.myLauncher.origin = null;
        hiWebOsFrame.createPage("VIDAAU2StatusPage", null, null, null, function (a) {
            hiWebOsFrame.VIDAAU2StatusPage = a;
        ...
        NoblexLauncherPageData.operateData.openMode = "input";
        NoblexLauncherRewrite();
        hiWebOsFrame.endLoading();
    })
}
function oneKeyOpenVIDAAU2LauncherInput() {
    DBG_INFO('oneKeyOpenVIDAAU2LauncherInput!!!');
    var crntPage = hiWebOsFrame.getCurrentPage();
    if (hiWebOsFrame.isCurrentModule("launcher")) {
        LogReportData.LauncherRunCloseReason=1;
        closePagesOrModuleByPage(crntPage);
        ...
        onKeySourceOpenVIDAAU2InputsPage(origin.id);
    }
    else {
        DBG_INFO('oneKeyOpenVIDAALauncherInput!!!');
        tryToCloseLauncher();
        if ('app' == crntPage.module) {
            DBG_INFO('oneKeyOpenVIDAALauncherInput!!!');
//            asyncStopApp(hiWebOsFrame.getCurrentPageId(),null);
            VIDAAU2LauncherCreateFromSourceKey = true;
            VIDAAU2LauncherCreateFromAllAppsKey = false;
            if(hiWebOsFrame.getCurrentBrand() == "nob"){
                hiWebOsFrame.createPage("NoblexLauncher_page", null, null, null, function (a) {
                    hiWebOsFrame.myLauncher = a;
                    hiWebOsFrame.myLauncher.operateData.callBackFunc = onkeySourceOpenNobLauncher;
                    asyncStopApp(hiWebOsFrame.getCurrentPageId(),hiWebOsFrame.myLauncher);
//                a.open();
//                a.hiFocus();
//                VIDAAU2LauncherCreateFromSourceKey = false;
...
//                onkeySourceOpenNobLauncher();
            ...
            hiWebOsFrame.createPage("VIDAAU2TvInput", null, null, null, function (a) {
                hiWebOsFrame.VIDAAU2TvInput = a;
                hiWebOsFrame.VIDAAU2TvInput.operateData.callBackFunc = onKeySourceOpenVIDAAU2InputsPage;
                asyncStopApp(hiWebOsFrame.getCurrentPageId(),hiWebOsFrame.VIDAAU2TvInput);
//                a.open();
//                a.hiFocus();
//                VIDAAU2LauncherCreateFromSourceKey = false;
            })
//            onKeySourceOpenVIDAAU2InputsPage(null);
        }
        else if (("setting" == crntPage.module || "settingfirst" == crntPage.module)
            && !!hiWebOsFrame.settingsFirst
            && !!hiWebOsFrame.settingsFirst.origin
            ...
            hiWebOsFrame.createPage("VIDAAU2TvInput", null, null, null, function (a) {
                closePagesOrModuleByPage(crntPage);
                hiWebOsFrame.VIDAAU2TvInput = a;
                asyncStopApp(oriPage.id, hiWebOsFrame.VIDAAU2TvInput);
                a.open();
                a.hiFocus();
                //VIDAAU2LauncherCreateFromSourceKey = false;
            })
...
var DeviceInfo = {
    ...
    createNew: function () {
        ...
        if (tv) {
            ...
            //deviceInfo.setModelName(getCurrentTVNO());  暂时注掉
...
function keyCinemaHandler() {
    try{
        DBG_INFO("cinema key--hiWebOsFrame.getCurrentPage().module:"+hiWebOsFrame.getCurrentPage().module);
        if(hiWebOsFrame.getCurrentPage().module == "wizard"){
            return
        }
        var sndCinema = 1 ;
        var picCinema = 4 ;
        tv && model.sound.setSoundMode(sndCinema);

        tv && model.video.setEnumPictureMode(picCinema);

        showTVTip("Cinema Mode");
    }
    catch (ex){
        DBG_ERROR(ex.message);
    }
}
function keyFootbalHandler(){
    try {
        DBG_INFO("football key--hiWebOsFrame.getCurrentPage().module:"+hiWebOsFrame.getCurrentPage().module);
        if(hiWebOsFrame.getCurrentPage().module == "wizard"){
            return
        }
        var PicMode={
...
//        var PicModeEnum=["[SE0004]", "[SE1413]", "[SE0007]", "[SE0009]", "[SE0464]", "[SE0008]"];
//        var SndModeEnum=["[SE0004]", "[SE0464]", "[SE0060]", "[SE0062]", "[SE0542]", "[SE1413]"];
//        var PicModeHDR=["[OT0095]", "[SE3368]", "[SE3279]", "[SE3367]", "[OT0274]","[OT0094]"];//["[OT0095]", "", "[SE3279]", "", "[OT0274]","[OT0094]"];
        var PicModeVec=["Standard", "Football", "Dynamic", "Natural", "Cinema","Game"];
        var SndModeEnum=["Standard", "Cinema", "Music", "Speech", "Late Night", "Football"];
...
//        if(!!isHDRFlag){
//            PicModeVec=PicModeHDR;
//        }else{
//            PicModeVec=PicModeEnum;
//        }
        var LastPicModeId = tv?model.video.getLastPicMode():0;
        DBG_INFO("model.video.getLastPicMode():"+LastPicModeId);
        var CurPicModeId = tv?model.video.getEnumPictureMode():2;//1
        DBG_INFO("model.video.getEnumPictureMode():"+CurPicModeId);

        var LastSndModeId = tv?model.sound.getLastSndMode():0;
        DBG_INFO("model.sound.getLastSndMode():"+LastSndModeId);
        var CurSndModeId = tv?model.sound.getSoundMode():5;//5
        DBG_INFO("model.sound.getSoundMode():"+CurSndModeId);

        if(CurPicModeId==PicMode.Football&&CurSndModeId==SndMode.Football){
            tv && model.video.setLastPicMode(CurPicModeId);
            DBG_INFO("model.video.setLastPicMode():"+CurPicModeId);
            tv && model.sound.setLastSndMode(CurSndModeId);
            DBG_INFO("model.sound.setLastSndMode():"+CurSndModeId);

            tv && model.video.setEnumPictureMode(LastPicModeId);
            DBG_INFO("model.video.setEnumPictureMode(1)");
            tv && model.sound.setSoundMode(LastSndModeId);
            DBG_INFO("model.sound.setSoundMode(5)");
            GLOBAL.PICTURE_MODE_TOAST_1 = getCurrentContentLanguage("Picture Mode")
                +":"+getCurrentContentLanguage(PicModeVec[LastPicModeId]);
            GLOBAL.PICTURE_MODE_TOAST_2 = getCurrentContentLanguage("Sound Mode")
                +":"+getCurrentContentLanguage(SndModeEnum[LastSndModeId]);
        }else {//if(CurPicModeId!=PicMode.Football&&CurSndModeId==SndMode.Football) {
            tv && model.video.setLastPicMode(CurPicModeId);
            DBG_INFO("model.video.setLastPicMode():" + CurPicModeId);
            tv && model.sound.setLastSndMode(CurSndModeId);
            DBG_INFO("model.sound.setLastSndMode():" + CurSndModeId);

            tv && model.video.setEnumPictureMode(PicMode.Football);
            DBG_INFO("model.video.setEnumPictureMode(1)");
            tv && model.sound.setSoundMode(SndMode.Football);
            DBG_INFO("model.video.setSoundMode(5)");
            GLOBAL.PICTURE_MODE_TOAST_1 = getCurrentContentLanguage("Picture Mode")
                +":"+getCurrentContentLanguage(PicModeVec[PicMode.Football]);
            GLOBAL.PICTURE_MODE_TOAST_2 = getCurrentContentLanguage("Sound Mode")
                +":"+getCurrentContentLanguage(SndModeEnum[SndMode.Football]);
        }
        $('#tv_tips_osd').css('width', 600 +"px");
        $('#tv_tips_osd').css('height', 200 +"px");
        $('#tv_tips_osd').css('margin-top', 830 +"px");
        $('#tv_tips_content').css('width', 600 +"px");
        $('#tv_tips_content_2').css('display', 'block');
        showTVPictureTip(GLOBAL.PICTURE_MODE_TOAST_1,GLOBAL.PICTURE_MODE_TOAST_2);
//        var Timer=null;
//        Timer = setTimeout(function () {
//            showTVTip(GLOBAL.PICTURE_MODE_TOAST_1);
//        },4000);
...
var OEMFunction = {
    ...
    createNew: function () {
        ...
        o.getLauncherTheme = function () {
            //return (DeviceInfo.getInstance().getBrandName()=="con");
            switch (DeviceInfo.getInstance().getBrandName()) {
                case "dev":
                case "pan":
                case "riv":
                case "kal":
    ...
    getConditionCheckResult: function (param) {
        ...
        switch (param) {
            ...
            case OEMFunction.OEMConditions.LAUNCHER_THEME:
                return o.getLauncherTheme();
