...
function Keyboard() {
    ...
    var KEYBOARD_KEYCODE_SCANCODE_MAP = {};
    {
        ...
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_PROGRAM_INFO] = VK_INFO;
        ...
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_SETUP] = VK_MENU;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_OK] = VK_ENTER;
        ...
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_OPERA] = VK_OPERA;
        ...
        SYSCMD_KEYCODE_MAP[guijs.KEYPAD_ONEKEY_OK] = VK_KEYPAD_ONEKEY_OK;
        SYSCMD_KEYCODE_MAP[guijs.KEYPAD_FIVEKEY_OK] = VK_KEYPAD_FIVEKEY_OK;
        SYSCMD_KEYCODE_MAP[guijs.KEYPAD_SEVENKEY_OK] = VK_KEYPAD_SEVENKEY_OK;
        ...
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_FOOTBAL] = VK_FOOTBAL;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_CINEMA] = VK_CINEMA;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_NEC_AUTO] = VK_NEC_AUTO;
    ...
    var onKey = function (key) {
        ...
        var isNeedOpenBacklight=false;
        // Get key state
        // var isKeyDown = !!( key.state & ( guijs.KEY_SINGLE | guijs.KEY_AUTOREP ) );
        // DBG_ERROR("KEY   :   " + SYSCMD_KEYCODE_MAP[key.command]);
        ...
        UI_keyState = key.state; //借用5658方案 UI.keyState 三个值 1 2 8，用于传递给cec设备

        DBG_ERROR("UI_keyState = "+UI_keyState);

        if (VK_POWER_ON == SYSCMD_KEYCODE_MAP[key.command] && (!!isKeyDown)) {
            recoverTVState();
            sendResult(key, true);
            return;
        }
...
//        }
        ...
        //add by jiaguili 20160920 for the child lock , do not reponse the key
        if (GLOBAL.PARENTAL_CHILD_LOCK && GLOBAL.PARENTAL_LOCK_SWITCH) {
            DBG_ALWAYS("recieve the key" + keyCode);
            if (ChILD_LOCK_KEYSET.indexOf(keyCode) < 0) {
                sendResult(key, true);
                g_toastType = ToastType.CHILD_LOCK;
        ...
        if (g_SystemAudioOnlyFlag == 1 && (keyCode != VK_MUTE && keyCode != VK_VOLUME_DOWN && keyCode != VK_VOLUME_UP)) {
            //chagne by jiauili , the key will trans to left and right key if setting is open
            isNeedOpenBacklight=true;
            if( (keyCode == VK_KEYPAD_VOLUME_DOWN || keyCode == VK_KEYPAD_VOLUME_UP)&&(!hiWebOsFrame.isCurrentModule("setting")&&!hiWebOsFrame.isCurrentModule("settingfirst"))){
                isNeedOpenBacklight=false;
            }
            if(isNeedOpenBacklight){
                try {
                    if (isRepeated) {
                        sendResult(key, true);
                        return;
    ...
    var handleKeyDown = function (keyCode, from) {
        ...
        UIObserver.publishMessage(UIObserver.MESSAGE_NAME.KEY_REPORT, keyCode);
    }
    ...
    var dispatchKeyEvent = function (type, keyCode, from) {
        // Create event
        debugE("timeout change source" + keyCode);
    ...
    function sendResult(key, val) {
        ...
        DBG_ERROR("send    key[" + SYSCMD_KEYCODE_MAP[key.command] + "]" + "["+key.command + "]"+
