...
function gingaApp() {
    ...
    self.GingaRefresh = GingaRefresh;
...
//    }
    ...
    function gingaAppEnterHandler() {
        try {
            ...
            var gingaId = ginga.pageData.tmp_app_list[this.SelectedIndex].id;
            if(gingaId != ""){
...
//                GingaScrollMoveTo(0);
                ...
                var appLength = ginga.pageData.tmp_app_list.length;
                var beginIdx = 0;
                if(firRunIdx - 4 <= 0) {
                    beginIdx = 0;
                } else {
    ...
    function refreshGingaApp(appVec) {   //appVec = [id, name, status]   status: uninit, avail, running, ''
        try {
            DBG_ERROR('refreshGingaApp: ' + appVec);
            var appTemp = {
                id: 'none',
                name: 'none',
                status: 'uninit',
            ...
            appTemp.user_visible = (parseInt(appVec[3])==1);

            var appIdx = _getIndexById(oprtData.appList, appTemp.id);
            if (-1 == appIdx) {
                DBG_ERROR('refreshGingaApp: appIdx = -1 appTemp.status = ' + appTemp.status);
                //if (appTemp.status == GINGA_STATE.AVAIL || appTemp.status == GINGA_STATE.RUNNING) {
                //    oprtData.appList.push(appTemp);
                //}
				oprtData.appList.push(appTemp);
            } else {
                if (appTemp.status == GINGA_STATE.UNINIT) {
                    DBG_ERROR('refreshGingaApp: status == UNINIT, splice it');
                    oprtData.appList.splice(appIdx, 1);
                } else {
                    DBG_ERROR('refreshGingaApp: status !== UNINIT, cloneObj');
    ...
    function printGingaApplist() {
        try {
            for (var i = 0; i < oprtData.appList.length; i++) {
                DBG_ERROR('ginga[' + i + ']:' + JSON.stringify(oprtData.appList[i]));
            }
        } catch (ex) {
            debugE(ex.message);
        }
    ...
    var onGingaVstrApplistChaged = function (val) {
        try {
            ...
            if (!!val && (val.length%4 == 0)) {
                var data_length = parseInt(val.length/4);
                for (var i=0;i<data_length;i++) {
                    refreshGingaApp(val.slice(i*4, i*4+4));
                }
    ...
    function getRunninGingaIndex() {
        var index = -1;
        try {
            for (var i = 0; i < ginga.pageData.tmp_app_list.length; i++) {
                if (GINGA_STATE.RUNNING == ginga.pageData.tmp_app_list[i].status) {
                    index = i;
                    break;
                }
            }
