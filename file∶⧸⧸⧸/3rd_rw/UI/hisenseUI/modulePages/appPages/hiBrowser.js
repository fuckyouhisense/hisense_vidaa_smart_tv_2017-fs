...
function AppHiBrowser() {
    ...
    self.onOpenHiBrowser = function () {
        ...
        if (oprtData.command.indexOf('http') > -1 || oprtData.command.indexOf('file:///') > -1) {
            startHiBrowser(oprtData.command);
            return;
        }

        if('SA' == hiWebOsFrame.getCurrentArea() && 'pan' == hiWebOsFrame.getCurrentBrand()) {
            sendAM(':am,opera4x,:resume=[-s,hibrowser,-u,http://www.carlosgutierrez.com.uy/internet-tv/index.html]');
            return;
        }
