        ...
        try {
            try {
                ...
                if (!firstInitUI) {
                    ...
                    GLOBAL.CURRENT_SVR_MSG = model.tvservice.getNoSignalMain();
                }
            ...
            if (!OadupgradMsgPoped) {
                ...
                //userMode && 修改成U M模式都能显示
                if (!!crntChannel && userMode && !!crntChannel.uid &&
                    firstPlay && "undefined" != typeof(livetvchlist)
...
//        }
    ...
    self.initLiveTVMain = function (afterInit) {
        ...
        if (tv) {
            ...
            //model.tvservice.onNoSignalMainChanged = signalChanged; 改用MessagePool方式
            UIObserver.subscribeMessage(UIObserver.MESSAGE_NAME.SIGNAL_CHANGED, signalChanged);  //liveTVSignalChanged;
            UIObserver.subscribeMessage(UIObserver.MESSAGE_NAME.CURRENT_SOURCE_CHANGED, parseCurrentSource);
            ...
            UIObserver.subscribeMessage(UIObserver.MESSAGE_NAME.PARENTAL_LOCK_SWITCH, onLockSwitchModeChanged);
            if ("EU" == InitArea) UIObserver.subscribeMessage(UIObserver.MESSAGE_NAME.ACTION_HDS_DISPLAY_CHANGED, onActionHdsAnswerchanged);
            //model.source.onInputCurrentInLockChaged = onCurrentInputLockChaged;  //父母锁消息，不再使用
    ...
    function mainPlayChanged(val) {
        DBG_ERROR("mainPlayChanged" + (new Date().getTime()));
        ...
        if (livetvchlist.getChannelChangeFlag() &&
            null != livetvchlist.getExpectedChannel() && !compareChannel(crntChannel, livetvchlist.getExpectedChannel())) {
        ...
        if (self.id != hiWebOsFrame.getCurrentPageId() || !hiWebOsFrame[LiveTVModule.MAIN].visible) {
            ...
            if (crntChannel.eCode != ECode.VIDEO_FMT_UPDATE && crntChannel.eCode != ECode.LOCKED &&
                ...
                if ("SA" == hiWebOsFrame.getCurrentArea()) {
                    //userMode && 修改成U M模式都能显示
                    if (firstPlay && userMode && crntChannel.ECODE == ECode.NONE) {
                        setOperateTipShowFlag(true);//add by ghl, 在operate_tip页面中关闭msg 和energy_warning不成功,直接用变量判断
                        openLiveTVSubPage(LiveTVModule.OPERATE_TIP);
                    }
                    else {
        ...
        try {
            changeChannelToStartRecord(crntChannel);
        }
        catch (ex) {
            DBG_ERROR(ex.message);
        }
    ...
    /**
     * 信号变化 liveTVSignalChanged
     * @param val
     */
    function signalChanged(val) {
        DBG_ERROR("signal changed to [" + val + "]");
        GLOBAL.CURRENT_SVR_MSG = val;
        if(val == 4) {
            resetOpenFlag();
            DBG_ALWAYS("signal changed to  lock");
        }
        //GLOBAL.CURRENT_SVR_MSG = val;
        try {
...
*/
            if(hiWebOsFrame.getCurrentPageId() == LiveTVModule.PASSWORD_DIALOG) { //父母锁已经打开
            ...
            switch (val) {
                ...
                case SvrMsg.NO_SIGNAL:
                    if (shouldPopUpMsg(Msg.SEARCH) && SourceList.TV == GLOBAL.CURRENT_SOURCE &&
                        hiWebOsFrame.isCurrentModule("livetv")) {
                        DBG_ERROR("open search dialog - signalChanged");
                ...
                case SvrMsg.LOCKED_INP:
                    if ((hiWebOsFrame.getCurrentPageId() == LiveTVModule.MAIN || hiWebOsFrame.isCurrentModule("input"))) {
                ...
                case SvrMsg.SCRAMBLED:
                    //showNoCamCardTip();
                    DBG_ERROR("SvrMsg.SCRAMBLED SA need add text");
                    showCIEncryptedMsg();//
                    break;
    ...
    function onInputRenamed(actionId, val) {
        ...
        var ret = objectFindByKey(sourceList, "uid", val[0]);
        if (null != ret) {
            ret.rename = val[1];
        } else {
    ...
    function onCecNotifyChangeSource(val) {
        ...
        try {
            ...
            if (tp_common.checkLeaveTimeshiftOrPvr(cecSourceStopPvrOrTShiftCallBack.bind(this, 0, val))) {
    ...
    function onActionHdsAnswerchanged(val){
      if(val == 0){
          //显示页面
          hiWebOsFrame.createPage('SettingCIVersionInfoPage', null, hiWebOsFrame.SettingChnlAdvancedPage, null, function (a) {
              DBG_INFO("SettingCIVersionInfoPage");
              hiWebOsFrame.SettingVersionInfo = a;
              a.open();
              a.hiFocus();
              a.rewriteDataOnly();
          });
          ...
          //退出页面
          if(hiWebOsFrame.getCurrentPageId() == "SettingCIVersionInfoPage"){
              hiWebOsFrame.SettingVersionInfo.close();
              hiWebOsFrame.SettingVersionInfo.origin.open();
              hiWebOsFrame.SettingVersionInfo.origin.hiFocus();
          }
      }
    }
    ...
    function parseSourceList(arr) {
        ...
        if (InitArea == "EU" && "5655_EU_MARKET" != currentPlatform_config) {
            ...
            //SA  0 tv, 1 av, 2 hdm1, 3 hdm2,  4 hdm3
            srcKey = [
                SourceList.TV,
                SourceList.AV,
                SourceList.HDMI1,
    ...
    function showOSDOnLiveTV(showInfo, chgChannel) {
        ...
        if (shouldPopUpMsg(Msg.ENERGY_WARNING)) {
            ...
            DBG_ERROR("open search dialog - showOSDOnLiveTV");
            openLiveTVSubPage(LiveTVModule.SEARCH_DIALOG);
        }
...
//        }
    ...
    function showNoSignalIcon(notSupport) {
        ...
        var totalMinutes = 14; //("SA" == crntArea) ? 14 : 14;
    ...
    function shouldPopUpMsg(name, showInfo, chgChannel) {
        ...
        switch (name) {
            ...
            case Msg.SIGNAL:
                ...
                break;
            case Msg.PASSWORD:
                //flag = parseLockState();  //兼容开机收不到消息单独读取锁状态
                var flags = (GLOBAL.CURRENT_SVR_MSG == SvrMsg.LOCKED_CH
                || GLOBAL.CURRENT_SVR_MSG == SvrMsg.LOCKED_PROG
                || GLOBAL.CURRENT_SVR_MSG == SvrMsg.LOCKED_INP); //parseLockState();
                DBG_ERROR("GLOBAL.CURRENT_SVR_MSG = " + GLOBAL.CURRENT_SVR_MSG);
                DBG_ERROR("flags = " + flags);

                globalLockState = flags;
                flag = globalLockState;
                break;
            case Msg.INFO:
...
//                break;
    ...
    function parseLockState() {
        ...
        DBG_ERROR({"message": "current channel", "data": crntChannel});
        //else {
    ...
    /**
     * UI自己切ui通道标记，禁止在未知通道时使用
     * 切台后一定会切到TV通道
     * @param innerId
     */
    self.setCurrentSourceInnerId = function (innerId) {
        currentSource = innerId; //currentSource = SourceList.TV ;
    }
    ...
    self.judgeToShowChannelSearch = function (val) {
        ...
        if (val && LiveTVModule.MAIN == hiWebOsFrame.getCurrentPageId() && SourceList.TV == currentSource) {
            DBG_ERROR("open search dialog - self.judgeToShowChannelSearch");
            openLiveTVSubPage(LiveTVModule.SEARCH_DIALOG);
        }
    }
