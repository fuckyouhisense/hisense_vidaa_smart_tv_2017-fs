...
function liveTVInfoBar() {
    ...
    var imgList = {
        ...
        HDMI1: 'img/infobar/5659UI/hdmi1.png',
        HDMI2: 'img/infobar/5659UI/hdmi2.png',
        HDMI3: 'img/infobar/5659UI/hdmi3.png',
        HDMI4: 'img/infobar/5659UI/hdmi4.png',
        AV: 'img/infobar/5659UI/av.png',
        Component: 'img/infobar/5659UI/component.png',
        ...
        if(!nosignal) {
            if(isDTV) {
                ...
                if("SA" == InitArea || "COL" == InitArea){
                    ...
                    var tempSAPTEXT = SAPTEXT[sap];
                    if(!!tempSAPTEXT) {
                        tempSAPTEXT = getCurrentContentLanguage(tempSAPTEXT)
                    }
                    oprtData.modeGroup[GroupT.SAP] = tempSAPTEXT;
                }
                else{
            ...
            if(!!videoInfo) {
                ...
                if (crntSource.innerId != SourceList.AV) {
                    ...
                    if("SA" == InitArea){
                        ...
                        $("#info_bar_not_tv span").eq(1).text(parseVideoInfo(livetvmain.getCurrentSourceInnerId() , videoInfo));
                    }
                    else{
    ...
    function parseVideoInfo(src, info) {
        ...
        if (SourceList.AV != src) {
            ...
            var colorSystem =  ((dpi[1] == "576") ? "[SE2164]" : "[SE2166]"); //彩色制式
            if(dpi.length == 6) {
                colorSystem = dpi[5];
            }
            txt = dpi[1] + (dpi[2] == 0 ? "i" : "p") + " " + colorSystem; // + ((dpi[1] == "576") ? "[SE2164]" : "[SE2166]");
        }
        if (!!dpi[3]) {
            txt = txt + " " + dpi[3] + "Hz";
        }
    ...
    function showRightPanel() {
        if(2 == pfInd) {
            ...
            var bkFlag = tv?tp_common.getProgramBookType(livetvmain.getCurrentChannelInfo(),prgrm.startTime,prgrm.endTime):{reminder:false, record: false};
            ...
            if(	!!$("#livetv_info_bar") &&	$("#livetv_info_bar").css("display") == "block") {
    ...
    function drowDolby() {
        if(dolbySpace == oprtData.modeGroup[GroupT.DOLBYICON]) {
           // $("#info_bar_group_info_bar_group_name_sys" + GroupT.DOLBYICON).removeAttr("style");
            $("#info_bar_group_info_bar_group_name_sys" + GroupT.DOLBYICON).css("background-image", "url(img/common/dolby.png)");
            $("#info_bar_group_info_bar_group_name_sys" + GroupT.DOLBYICON).css("width", "30px");
            $("#info_bar_group_info_bar_group_name_sys" + GroupT.DOLBYICON).css("color", "transparent");
        }
        else {
            $("#info_bar_group_info_bar_group_name_sys" + GroupT.DOLBYICON).removeAttr("style");
           // $("#info_bar_group_info_bar_group_name_sys" + GroupT.DOLBYICON).css("display","none");
        }
    }
    ...
    function rewritePageData(pageData) {
        try {
            ...
            var d = firstCreate ? {} : getSystemTime();
            if("SA" == hiWebOsFrame.getCurrentArea()) {
                pageData.info_bar_time.Data = d.date + " " + d.time;
            } else {
            ...
            pageData.info_bar_nxt_rating.Data = "";//oprtData.nxtRating ? oprtData.nxtRating : ""; MT5659SA-1806 songqian
    ...
    function initInfoBarData(chInfo, flipInfo) {
        ...
        if(!flipInfo) {
             ...
             */
            oprtData.signalLevel = tv ? model.servicemode.getSignalMainLevels()[0] : "10";
            oprtData.signalQuality = tv ? model.servicemode.getTunersignalinfoSignalqualities()[0] : "10";
    ...
    function setVideoAudioInfo() {
        ...
        if(!!audioInfo) {
            ...
            if(1 == aiArr.length) {
                ...
                if(!!aiArr[1] &&(null != aiArr[1].match("ac3") || null != aiArr[1].match("AC3")
                        || null != aiArr[1].match("ac4") || null != aiArr[1].match("AC4")
                        || null != aiArr[1].match("atmos") || null != aiArr[1].match("ATMOS")
                    )) {//dolby
                    oprtData.modeGroup[GroupT.DOLBY] = aiArr[1];
                    oprtData.modeGroup[GroupT.DOLBYICON] = dolbySpace;
                }
                else {
                    oprtData.modeGroup[GroupT.DOLBYICON] = "";
                    oprtData.modeGroup[GroupT.DOLBY] = aiArr[1];
                }
                drowDolby();
            }
