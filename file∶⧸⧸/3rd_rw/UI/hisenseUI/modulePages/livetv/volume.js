...
function liveTVVolume() {
    ...
    self.onOpenVolume = function () {
        ...
        if (!onlyOpen) {
            DBG_ERROR("onOpenVolume model.sound.getMainMute() = " + model.sound.getMainMute());
    ...
    self.setBizVolume = function (v) {
        ...
        var speakerState = tv ? model.sound.getTvSpeaker() : 1;
        if (isCECByPass() && 0 == speakerState) {
            //keyboard.SendKeyResult(false);
            DBG_ERROR("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2 = isCECByPass");
            var keyCode = hiWebOsFrame.getKeyStackTop();
    ...
    function setPageMute(val) {
        ...
        isMute = !!val;
        DBG_ERROR("setPageMute isMute = " + isMute);
        displayOfMute(isMute);
    }
    ...
    function displayOfMute(val) {
        DBG_ERROR("displayOfMute val = " + val);
        DBG_ERROR("displayOfMute volume_mute visibility" + $("#volume_mute").css("visibility"));
        if (!isEPOSON()) {
            var v = val ? "visible" : "hidden";
            $("#volume_mute").css("visibility", v);
            DBG_ERROR("displayOfMute -1 ! isEPOSON()");
        }
        else{
            if(!val && "hidden" != $("#volume_mute").css("visibility")){
                $("#volume_mute").css("visibility", "hidden");
                DBG_ERROR("displayOfMute -3  mute");
            }
        }
        DBG_ERROR("displayOfMute volume_mute visibility" + $("#volume_mute").css("visibility"));
    }
    ...
    self.reCheckVolume = function () {
        ...
        if (canProcessVolume()) {
            DBG_ERROR("reCheckVolume model.sound.getMainMute() = " + model.sound.getMainMute());
