function EPG() {
    ...
    var flipFlag = FLIPMODE.NONE, backStartTime = [],backEndTime = [],requestOut = [];
    var mediaBaseURL = "", mediaTimeout = 0, enableThemeColor = true;
    var backChannelIndex = -1;
    ...
    function getProgramEvent( noDelayFlag ) {
        try {
            ...
            if (ENABLE_FVP && baseLine > startLine) {
                ...
                DBG_ALWAYS('fvp serviceIds:'+serviceIds+",playIds:"+playIds+ ",startLine:"+startLine +",endLine" +endLine);
                if (!!serviceIds) {
    ...
    function checkReadedCount( cnt, uid , EPGOnMerge) {
        readedCount = cnt;
            ...
            if(EPGOnMerge){
                unLockEPGPageKeys('merge');
            }else{
                unLockEPGPageKeys("getProgramEvent");
            }
            showEPGPage();
    ...
    function onGetFvpNotMergePrograms( fvpEvents ) {
        ...
        for (i = 0; i < channelMaxRows; i++) {
            ...
            if (tv) {
                if (!chnl || "" == chnl.serviceId ) {
                    DBG_ERROR("chnl:"+objToString(chnl));
                    continue;
                }
...
function doHTMLFilter(str, fullFilter) {
    ...
    function getIsPvrOrReminder( channel, startTime, endTime ) {
        try {
            var bookFlag = { reminder: false, record: false };
            if(!tv){
                return bookFlag;
            }
            var flag = tp_common.getProgramBookType(channel, startTime, endTime);
            switch (parseInt(flag)) {
                case tp_common.PVR_BOOK_TYPE.PVR:
                    bookFlag.record = true;
                    break;
                case tp_common.PVR_BOOK_TYPE.REMINDER:
                    bookFlag.reminder = true;
                    break;
                default :
                    bookFlag.record = false;
                    bookFlag.reminder = false;
                    break;
            }
            return bookFlag;
        } catch ( ex ) {
            DBG_ERROR('getIsPvrOrReminder is error:' + ex.message)
        }
    }
    ...
    function rewritePageData(pageData) {
        ...
        var bookFlag = getIsPvrOrReminder(chnl,cur_prgrm.startTime,cur_prgrm.endTime);
        ...
        try {
            pageData.epg_opertion_btn.Data[2].flip_text.Data = enableThemeColor ? "Program Colour" : "";
            if(!enableThemeColor){
                pageData.epg_opertion_btn.Data[2].flip_img.Data = imgList.noImg;
            }else{
                pageData.epg_opertion_btn.Data[2].flip_img.Data = imgList.CHFLIP;
            }
        }
        catch (ex) {
        ...
        pageData.epg_program_time.Data = getProgramLocalTime(oprtData.channels[channelIndex].program[programIndex].startTime, oprtData.channels[channelIndex].program[programIndex].endTime,5);
        ...
        for (i = 0; i < channelMaxRows; i++) {
            ...
            if (chnl) {
                pageData.epg_channel_list.Data.push({
                    ...
                    channel_fav: {Data: chnl.favType ? imgList.fav : imgList.noImg, disable: false},
            ...
            for (j = 0; j < programMaxCols; j++) {
                ...
                if (prgrm) {
                    bookFlag = getIsPvrOrReminder(chnl,prgrm.startTime,prgrm.endTime);
    ...
    self.onOpen = function () {
        ...
        setTimeout(tp_common.updateScheduleList, 500);
    }
    ...
    function removeAvailableIcon(chIdx, prgrmIdx) {
        ...
        if(durationTimeToWidth(rmSelProgram.endTime - rmSelProgram.startTime) > 150){
    ...
    self.keyRightHandler = function () {
        ...
        if (crntSelProgram.endTime >= endLine - 60 || programMaxCols - 1 == this.SelectedIndex) {
    ...
    self.EPGNumKeyMapProcess=function (keyValue){
        try{
            if("SA" == hiWebOsFrame.getCurrentArea()){
                DBG_INFO("SA ,disable num key locate");
                return;
            }
            var num = keyValue;
            inputedNum += (num + "");
    ...
    self.pageData = {
        ...
        epg_opertion_btn: {
            Data: [{
                ...
                flip_text: {Data: "EU" == InitArea ? "Program Colour" : ""},
